...
model.load_weights("inceptionv3_weights_all.best.hdf5")
...
test_generator = test_datagen.flow_from_directory(test_dir,target_size=(299, 299),batch_size=30,class_mode=None, shuffle = False)
...
plt.title('Inception v3', fontsize =15, fontweight ='bold')
...
#Plotting ROC curves. The code is mainly adapted from the following link
...
#             ''.format(i, roc_auc[i]))
plt.plot(fpr[0], tpr[0], color='blue', lw=lw, label='MAD (area = {:.2f})'.format(roc_auc[0]))
plt.plot(fpr[1], tpr[1], color='green', lw=lw, label='CNV (area = {:.2f})'.format(roc_auc[1]))
plt.plot(fpr[2], tpr[2], color='red', lw=lw, label='DME (area = {:.2f})'.format(roc_auc[2]))
plt.plot(fpr[3], tpr[3], color='cyan', lw=lw, label='NORMAL (area = {:.2f})'.format(roc_auc[3]))
