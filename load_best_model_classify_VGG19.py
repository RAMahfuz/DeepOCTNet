...
model.load_weights("vgg19_weights_all.best.hdf5")
...
plt.title('VGG19', fontsize =15, fontweight ='bold')
...
plt.plot(fpr[1], tpr[1], color='green', lw=lw, label='CNVE (area = {:.2f})'.format(roc_auc[1]))
...
#plt.plot(fpr[4], tpr[4], color='magenta', lw=lw, label='mel (area = {:.2f})'.format(roc_auc[4]))
#plt.plot(fpr[5], tpr[5], color='yellow', lw=lw, label='nv (area = {:.2f})'.format(roc_auc[5]))
#plt.plot(fpr[6], tpr[6], color='black', lw=lw, label='vasc (area = {:.2f})'.format(roc_auc[6]))
